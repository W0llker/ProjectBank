...
public class ClientController {
    ...
    public ClientController(RepositoryUser repositoryUser, RepositoryAccount repositoryAccount, RepositoryOperation repositoryOperation, UsersValidator usersValidator) {
    ...
    public void clientMenu(Scanner scanner, Users users) throws ExceptionExit {
        ...
        try {
            ...
            switch (numberAction) {
                case "1":
                    System.out.println(repositoryAccount.getAccountsToUser(users.getId()));
                    System.out.println("Выберите счет валюты которую меняете");
                    long idAc = scanner.nextLong();
                    repositoryAccount.getAccountToUser(idAc, users.getId());
                    System.out.println("Введите счет валюты которую получайте");
                    long idRecipient = scanner.nextLong();
                    repositoryAccount.getAccountToUser(idRecipient, users.getId());
                    if (idAc == idRecipient) {
                        throw new ExceptionUsers("Нельзя обменять одну и туже валюту");
                    }
                ...
                case "2":
                    ...
                    repositoryAccount.checkAccount(idRecipient, users.getId());
                    confirmationTranslation(idRecipient, scanner);
                ...
                case "3":
                    ...
                    break;
                case "4":
                    System.out.println(currencyRepository.deserializationCurrency());
                    break;
                case "5":
                ...
                case "6":
                    ...
                    clientService.payment(accounts, value, users);
                    break;
                case "7":
                    System.out.println(repositoryAccount.getAccountsToUser(users.getId()));
                    System.out.println("Введите id счета который хотите удалить");
                    id = scanner.nextLong();
                    repositoryAccount.getAccountToUser(id, users.getId());
                ...
                case "9":
                    ...
                    break;
                case "10":
                    System.out.println("Ваш счет в BYN= " + clientService.convertValueTOBYN(repositoryAccount.getAccountsToUser(users.getId())));
                    break;
                case "11":
                    System.out.println(repositoryAccount.getAccountsToUser(users.getId()));
    ...
    private void confirmationTranslation(long id, Scanner scanner) throws ExceptionExit {
        Accounts accounts = repositoryAccount.checkAccount(id);
